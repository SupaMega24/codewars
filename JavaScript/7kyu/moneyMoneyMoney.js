...
const calculateYears = (principal, interest, tax, desired) => {
    if (principal >= desired) {
        return 0; // If desired amount already reached, no need to wait
    }

    // Formula to calculate the number of years needed
    const years = Math.ceil(Math.log(desired / principal) / Math.log(1 + interest * (1 - tax)));
...
//     // Loop until the principal amount reaches or exceeds the desired sum
